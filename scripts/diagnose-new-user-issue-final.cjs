...
const BUY_TICKET_SIG = ethers.id("BuyTicket(address,uint256,uint256,uint256)");
const GET_REWARD_SIG = ethers.id("GetReward(address,uint256,uint256,uint256)");
...
async function diagnoseNewUserIssueV2() {
  ...
  try {
    ...
    for (const log of buyTicketEvents) {
      try {
        // BuyTicket(address user, uint256 amount, uint256 ticketCount, uint256 timestamp)
        // indexed user =>  topics[1]
        // data是剩余的参数(amount, ticketCount, timestamp)
        const userAddr = "0x" + log.topics[1].slice(-40);
        const dataHex = log.data;

        // 解析data中的3个uint256
        const amount = BigInt("0x" + dataHex.slice(2 + 0 * 64, 2 + 1 * 64));
        const ticketCount = BigInt("0x" + dataHex.slice(2 + 1 * 64, 2 + 2 * 64));
        const timestamp = BigInt("0x" + dataHex.slice(2 + 2 * 64, 2 + 3 * 64));
    ...
    console.log(
      `✓ 已解析购票用户：${Object.keys(userInputMap).length} 个\n`
    );
    ...
    for (const log of getRewardEvents) {
      try {
        // GetReward(address user, uint256 amount, uint256 rewardType, uint256 timestamp)
        const userAddr = "0x" + log.topics[1].slice(-40);
        const dataHex = log.data;

        const amount = BigInt("0x" + dataHex.slice(2 + 0 * 64, 2 + 1 * 64));
        const rewardType = BigInt(
          "0x" + dataHex.slice(2 + 1 * 64, 2 + 2 * 64)
        );
        const timestamp = BigInt("0x" + dataHex.slice(2 + 2 * 64, 2 + 3 * 64));
    ...
    console.log(
      `✓ 已解析奖励用户：${Object.keys(userOutputMap).length} 个\n`
    );
    ...
    console.log(
      `无收益率：${(
        (noRewardUsers.length / Object.keys(userInputMap).length) *
        100
      ).toFixed(1)}%\n`
    );
    ...
    if ((analysis.firstPurchaseWithin30Days / sampleSize) > 0.7) {
      console.log("1️⃣ 时间滞后效应强劲");
      console.log(
        `   ${(
          (analysis.firstPurchaseWithin30Days / sampleSize) *
          100
        ).toFixed(0)}% 的无收益用户最近30天内才购票`
      );
      console.log("   → 原因：购票后的奖励需要时间兑现（7/15/30天周期）");
      console.log(
        "   → 这可能是正常现象，再等一段时间就会获得奖励\n"
      );
    }

    if ((analysis.firstPurchaseWithin7Days / sampleSize) > 0.4) {
      console.log("2️⃣ 很多用户才购票几天");
      console.log(
        `   ${(
          (analysis.firstPurchaseWithin7Days / sampleSize) *
          100
        ).toFixed(0)}% 的无收益用户只购票7天以内`
      );
      console.log("   → 原因：时间周期还没到期\n");
    }

    if ((analysis.singlePurchaseUsers / sampleSize) > 0.8) {
      console.log("3️⃣ 单次购票用户占主体");
      console.log(
        `   ${(
          (analysis.singlePurchaseUsers / sampleSize) *
          100
        ).toFixed(0)}% 是单次购票`
      );
      console.log(
