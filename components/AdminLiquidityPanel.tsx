...
const AdminLiquidityPanel: React.FC = () => {
  ...
  const handleAddLiquidity = async () => {
    ...
    try {
      ...
      // 检查并授权JBC代币 - 简化流程，减少RPC调用
      if (jbcAmountWei > 0n) {
        console.log('🔍 [AdminLiquidityPanel] 检查JBC授权...');
        try {
          ...
          if (jbcAllowance < jbcAmountWei) {
            console.log('⚠️ [AdminLiquidityPanel] JBC授权不足，开始授权...');
            ...
            try {
              ...
              console.log('✅ [AdminLiquidityPanel] JBC授权成功，区块:', receipt.blockNumber);
            ...
            console.log('✅ [AdminLiquidityPanel] JBC已授权，授权额度充足');
          }
        } catch (error: any) {
          console.error('❌ [AdminLiquidityPanel] 检查JBC授权失败:', error);
          toast.error('检查JBC授权失败，请刷新页面后重试');
          setIsLoading(false);
          return;
        }
      }
      ...
      try {
        const gasEstimate = await protocolContract.addLiquidity.estimateGas(jbcAmountWei, txParams);
        txParams.gasLimit = gasEstimate * 120n / 100n; // 增加20%的Gas缓冲
        console.log('⛽ [AdminLiquidityPanel] Gas estimate:', gasEstimate.toString());
      } catch (gasError: any) {
        console.warn('⚠️ [AdminLiquidityPanel] Gas估算失败，使用默认值:', gasError.message);
        // 使用默认的 Gas limit
        txParams.gasLimit = 500000n;
      }
