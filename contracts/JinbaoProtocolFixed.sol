...
contract JinbaoProtocolFixed is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // 初始化缺失的状态变量
    function initializeMissingStates() external onlyOwner {
        if (redemptionFeePercent == 0) redemptionFeePercent = 1;
        if (!liquidityEnabled && !redeemEnabled) {
            liquidityEnabled = true;
            redeemEnabled = true;
        }
        if (directRewardPercent == 0) {
