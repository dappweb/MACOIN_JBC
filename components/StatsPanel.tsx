...
const StatsPanel: React.FC<StatsPanelProps> = ({ stats: initialStats, onJoinClick, onWhitepaperClick }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-fade-in">
      ...
      <div className="relative rounded-2xl md:rounded-3xl overflow-hidden min-h-[250px] md:min-h-[300px] flex items-center bg-gradient-to-br from-yellow-300 via-amber-400 to-yellow-600 border border-yellow-500/50 shadow-2xl">
         {/* Texture Overlay */}
         ...
         <div className="relative z-10 p-5 sm:p-6 md:p-12 max-w-2xl w-full">
            ...
            <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-6xl font-extrabold text-slate-900 mb-3 md:mb-4 leading-tight drop-shadow-sm">
                {t.stats.title} <br/>
                <span className="text-white drop-shadow-md text-xl sm:text-2xl md:text-3xl lg:text-5xl">{t.stats.subtitle}</span>
            </h1>
      ...
      <div className="glass-panel p-4 sm:p-5 md:p-6 rounded-xl md:rounded-2xl bg-white border-l-4 border-macoin-500 flex flex-col items-start sm:items-center gap-4 shadow-sm">
        ...
        <div className="flex flex-col sm:flex-row w-full sm:w-auto gap-2">
            {!isConnected ? (
                <button disabled className="px-4 py-2.5 md:px-6 md:py-3 bg-slate-200 text-slate-400 font-bold rounded-lg cursor-not-allowed whitespace-nowrap text-sm md:text-base w-full sm:w-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        <div className="glass-panel p-4 md:p-6 rounded-xl md:rounded-2xl hover:border-macoin-500/40 transition-colors bg-white">
            ...
            <div className="text-2xl md:text-3xl font-bold text-slate-900 mb-1">{displayStats.balanceJBC.toLocaleString()}</div>
        ...
        <div className="glass-panel p-4 md:p-6 rounded-xl md:rounded-2xl hover:border-macoin-500/40 transition-colors bg-white">
            ...
            <div className="text-2xl md:text-3xl font-bold text-slate-900 mb-1">{displayStats.totalRevenue.toLocaleString()}</div>
      ...
      <div className="glass-panel p-4 md:p-6 rounded-xl md:rounded-2xl bg-white">
         <h3 className="text-base md:text-lg font-bold mb-4 md:mb-6 text-slate-900 border-l-4 border-macoin-500 pl-3">{t.stats.chartTitle}</h3>
