# 极差奖励机制设计文档

## 📋 需求概述

根据白皮书第四节"极差裂变机制"，实现V1-V9的极差奖励分配。

---

## 🎯 核心逻辑

### 1. 平级判断规则

**关键点**：
- 极差奖励向上遍历推荐链
- **只有当上级的等级 > 下级的等级时，上级才能获得极差奖励**
- 极差奖励 = (上级等级比例 - 下级等级比例) × 门票金额

### 2. 等级系统

| 等级 | 要求有效地址数 | 极差奖励比例 |
|------|--------------|------------|
| V0   | 0            | 0%         |
| V1   | 10           | 5%         |
| V2   | 30           | 10%        |
| V3   | 100          | 15%        |
| V4   | 300          | 20%        |
| V5   | 1000         | 25%        |
| V6   | 3000         | 30%        |
| V7   | 10000        | 35%        |
| V8   | 30000        | 40%        |
| V9   | 100000       | 45%        |


## 📊 案例分析

### 案例1：同一条线的平级问题

**场景**：
```
你 (V1, 10个直推)
 └─ A (你的直推, V2, 30个有效地址)
     └─ B (A的直推, 购买100MC门票)
```

**问题**：你和A在同一条推荐线上，A的等级比你高，你是否能获得A团队的极差奖励？

**答案**：❌ **不能**

**原因**：
- 你是V2 没有超过100个地址 (你的地址+ A的30个有效地址，10%)
- A是V2 (10%)
- 你的等级 <= A的等级
- 极差 = 10% - 10% = 0% (负数)
- **极差 = 0，不获得奖励**

### 案例2：你升级后反超

**场景**：
```
你 (V3, 100个有效地址) ← 升级了！
 └─ A (V2, 30个有效地址)
     └─ B (购买100MC门票)
```

**结果**：✅ **你获得极差奖励**

**计算**：
```
B购买门票: 100 MC

1. 直推奖励: 25 MC → A

2. 极差奖励:
   - A是V2 (10%)，获得 10% × B的静态收益（解冻后）
   - 但A已经拿了25MC门票直推奖励，极差是否叠加？不叠加
   ⚠️ 重要问题：极差奖励是基于什么金额计算？ 10% × B的静态收益（解冻后）

```

### 案例3：多层级极差分配

**场景**：
```
V5大佬 (25%) ← 最上级
 └─ V3你 (15%)
     └─ V2-A (10%)
         └─ V1-B (5%)
             └─ V0-C (购买100MC门票)
```

**完整奖励分配**：

```
C购买100MC门票：

1. 直推奖励 (25%):
   - C的直推人B获得 25 MC

2. 层级奖 (15%):
   - 根据推荐人数计算
    推荐N人，拿5N层，每层1%（门票入金MC）。
    推荐1人拿五层，推荐2人拿10层，推荐3人拿15层

3. 极差奖励 (逐层计算):

   第1层：C (V0, 0%) → B (V1, 5%)
   - B获得: (5% - 0%) × C静态收益（解冻后）

   第2层：B (V1, 5%) → A (V2, 10%)
   - A获得: (10% - 5%) ×  C静态收益（解冻后）

   第3层：A (V2, 10%) → 你 (V3, 15%)
   - 你获得: (15% - 10%) ×  C静态收益（解冻后）

   第4层：你 (V3, 15%) → V5大佬 (25%)
   - 大佬获得: (25% - 15%) ×  C静态收益（解冻后）

   总极差奖励: 5 + 5 + 5 + 10 = 25 MC
```

---

## 🔧 技术实现

### 1. 数据结构修改

```solidity
// 在 JinbaoProtocol.sol 中添加

struct UserInfo {
    address referrer;        // 推荐人
    uint256 activeDirects;   // 有效直推数
    uint256 teamCount;       // 团队总人数
    uint256 totalRevenue;    // 累计收益
    uint256 currentCap;      // 当前收益上限
    bool isActive;           // 是否激活
    uint8 level;            // V0-V9等级 (新增)
}

// 等级配置
struct LevelConfig {
    uint256 requiredAddresses;  // 需要的有效地址数
    uint8 rewardPercent;        // 极差奖励比例
}

mapping(uint8 => LevelConfig) public levelConfigs;
```

### 2. 核心函数

#### A. 获取用户等级

```solidity
function getLevel(address user) public view returns (uint8) {
    uint256 activeDirects = userInfo[user].activeDirects;

    if (activeDirects >= 100000) return 9;  // V9
    if (activeDirects >= 30000) return 8;   // V8
    if (activeDirects >= 10000) return 7;   // V7
    if (activeDirects >= 3000) return 6;    // V6
    if (activeDirects >= 1000) return 5;    // V5
    if (activeDirects >= 300) return 4;     // V4
    if (activeDirects >= 100) return 3;     // V3
    if (activeDirects >= 30) return 2;      // V2
    if (activeDirects >= 10) return 1;      // V1
    return 0;  // V0
}
```

#### B. 获取等级对应的奖励比例

```solidity
function getLevelPercent(uint8 level) public pure returns (uint8) {
    if (level == 9) return 45;  // V9: 45%
    if (level == 8) return 40;  // V8: 40%
    if (level == 7) return 35;  // V7: 35%
    if (level == 6) return 30;  // V6: 30%
    if (level == 5) return 25;  // V5: 25%
    if (level == 4) return 20;  // V4: 20%
    if (level == 3) return 15;  // V3: 15%
    if (level == 2) return 10;  // V2: 10%
    if (level == 1) return 5;   // V1: 5%
    return 0;  // V0: 0%
}
```

#### C. 分配极差奖励（核心逻辑）

```solidity
function _distributeDifferentialRewards(
    address buyer,
    uint256 ticketAmount
) internal {
    address current = userInfo[buyer].referrer;
    uint8 previousLevel = 0;  // 从最底层开始

    // 向上遍历推荐链，最多遍历20层
    for (uint256 i = 0; i < 20 && current != address(0); i++) {
        uint8 currentLevel = getLevel(current);

        // 只有当当前等级 > 之前的等级时，才发放极差奖励
        if (currentLevel > previousLevel) {
            uint8 previousPercent = getLevelPercent(previousLevel);
            uint8 currentPercent = getLevelPercent(currentLevel);

            // 极差比例
            uint8 differential = currentPercent - previousPercent;

            // 计算奖励金额
            uint256 rewardAmount = (ticketAmount * differential) / 100;

            // 检查收益上限
            if (userInfo[current].totalRevenue + rewardAmount <= userInfo[current].currentCap) {
                // 发放MC奖励
                mcToken.transfer(current, rewardAmount);
                userInfo[current].totalRevenue += rewardAmount;

                emit DifferentialRewardPaid(current, buyer, rewardAmount, currentLevel);
            }

            // 更新为当前等级，用于下一层比较
            previousLevel = currentLevel;
        }

        // 移动到上一级推荐人
        current = userInfo[current].referrer;
    }
}
```

#### D. 在购买门票时调用

```solidity
function buyTicket(uint256 amount) external nonReentrant {
    // ... 现有逻辑

    // 1. 直推奖励 (25%)
    if (referrerAddr != address(0)) {
        uint256 directReward = (amount * 25) / 100;
        mcToken.transfer(referrerAddr, directReward);
    }

    // 2. 极差奖励 (5%-45%)
    _distributeDifferentialRewards(msg.sender, amount);

    // 3. 层级奖 (待实现)
    // _distributeLayerRewards(msg.sender, amount);

    // ... 其他逻辑
}
```

### 3. 事件定义

```solidity
event DifferentialRewardPaid(
    address indexed receiver,
    address indexed buyer,
    uint256 amount,
    uint8 level
);
```

---

## 🧪 测试用例

### 测试1：基本极差计算

```javascript
it("应该正确计算单层极差奖励", async function () {
    // 设置：V1推荐V0
    // V1有10个有效地址，V0购买门票

    // 预期：V1获得5%极差奖励
    const ticketAmount = ethers.parseEther("100");
    const expectedReward = ethers.parseEther("5"); // 5%

    // ... 测试代码
});
```

### 测试2：平级不发放奖励

```javascript
it("平级时不应发放极差奖励", async function () {
    // 设置：V2推荐V2
    // 两个用户都是V2等级

    // 预期：上级V2不获得极差奖励

    // ... 测试代码
});
```

### 测试3：多层级极差分配

```javascript
it("应该正确分配多层级极差奖励", async function () {
    // 设置：V5 -> V3 -> V1 -> V0(购买者)

    // 预期：
    // V1获得: 5% - 0% = 5%
    // V3获得: 15% - 5% = 10%
    // V5获得: 25% - 15% = 10%

    // ... 测试代码
});
```

### 测试4：收益上限限制

```javascript
it("达到收益上限后不应继续发放极差奖励", async function () {
    // 设置：用户已接近3倍收益上限

    // 预期：只发放到上限为止，超出部分不发放

    // ... 测试代码
});
```

---

## ⚠️ 重要注意事项

### 1. 极差奖励的计算基数

**问题**：极差奖励应该基于什么金额计算？

**选项A**：基于门票原始金额（100 MC）
```
直推奖励: 25%
极差奖励: 基于100%原始金额
总计可超过100%
```

**选项B**：基于剩余金额（扣除直推后）
```
直推奖励: 25% (25 MC)
极差奖励: 基于剩余75 MC
```

**建议**：采用**选项A**（基于原始金额），因为：
1. 更符合传统MLM逻辑
2. 白皮书中各项奖励比例相加超过100%
3. 合约需要预先注入足够的MC作为奖励池

### 2. 奖励资金来源

极差奖励的MC从哪里来？

**选项A**：从门票金额中扣除
```
问题：门票金额可能不够分配所有奖励
```

**选项B**：从奖励池中发放
```
✅ 推荐：合约预先注入MC作为奖励池
类似现有的 claimRewards() 逻辑
```

### 3. 推荐关系绑定

**问题**：如何避免循环推荐？

```solidity
function bindReferrer(address referrer) external {
    require(userInfo[msg.sender].referrer == address(0), "Already bound");
    require(referrer != msg.sender, "Cannot refer self");

    // ✅ 添加：检查循环推荐
    require(!_isInReferralChain(referrer, msg.sender), "Circular referral");

    userInfo[msg.sender].referrer = referrer;
}

function _isInReferralChain(address user, address target) internal view returns (bool) {
    address current = user;
    for (uint i = 0; i < 20 && current != address(0); i++) {
        if (current == target) return true;
        current = userInfo[current].referrer;
    }
    return false;
}
```

---

## 📌 实施步骤

### 阶段1：数据结构（1天）
- [ ] 添加 `level` 字段到 `UserInfo`
- [ ] 实现 `getLevel()` 函数
- [ ] 实现 `getLevelPercent()` 函数

### 阶段2：核心逻辑（2天）
- [ ] 实现 `_distributeDifferentialRewards()` 函数
- [ ] 在 `buyTicket()` 中集成极差奖励
- [ ] 添加事件日志

### 阶段3：测试（2天）
- [ ] 编写单元测试
- [ ] 测试各种边界情况
- [ ] 测试多层级分配

### 阶段4：部署和验证（1天）
- [ ] 部署到测试网
- [ ] 前端集成显示
- [ ] 用户测试验证

---

## 🎯 FAQ

### Q1: 如果推荐链中有人等级相同怎么办？

**A**: 跳过该层，不发放极差奖励，继续向上查找更高等级。

**示例**：
```
V3 -> V2 -> V2 -> V1 -> V0(购买者)
              ↑
           跳过这层
```

奖励分配：
- V1获得: 5% - 0% = 5%
- 第一个V2获得: 10% - 5% = 5%
- 第二个V2: 跳过（平级）
- V3获得: 15% - 10% = 5%

### Q2: 最多能获得多少极差奖励？

**A**: 理论上，如果你是V9（45%），直推是V0（0%），你能获得45%的极差奖励。

但实际上，奖励会被中间各层分摊：
```
V9(45%) -> V5(25%) -> V0
V5获得: 25%
V9获得: 45% - 25% = 20%
```

### Q3: 极差奖励和收益上限的关系？

**A**: 极差奖励计入 `totalRevenue`，受3倍门票金额的收益上限限制。

达到上限后，该用户不再获得任何收益（包括极差奖励），需要重新购买门票激活。

---

**更新时间**: 2025-12-18
**版本**: v1.0.0
**状态**: 设计文档
