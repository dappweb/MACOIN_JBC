...
contract JinbaoProtocolV4 is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function redeem() external nonReentrant {
        ...
        if (totalYield > 0) {
            ...
            if (totalYield > 0) {
                ...
                // 先计算静态奖励的MC和JBC（但不立即分配）
                uint256 staticMCPart = totalYield / 2;
                uint256 staticJBCValuePart = totalYield / 2;
                uint256 jbcPrice = _getCurrentJBCPrice();
                uint256 staticJBCAmount = (staticJBCValuePart * 1 ether) / jbcPrice;
                
                // 分配静态奖励
                uint256 mcTransferred = 0;
                if (address(this).balance >= staticMCPart && staticMCPart > 0) {
                    _transferNativeMC(msg.sender, staticMCPart);
                    mcTransferred = staticMCPart;
                }
                
                uint256 jbcTransferred = 0;
                if (jbcToken.balanceOf(address(this)) >= staticJBCAmount && staticJBCAmount > 0) {
                    jbcToken.transfer(msg.sender, staticJBCAmount);
                    jbcTransferred = staticJBCAmount;
                }
    ...
    function _calculateAndStoreDifferentialRewardsFromStatic(address user, uint256 staticMC, uint256 staticJBC, uint256 stakeId) internal {
        ...
        while (current != address(0) && iterations < TokenomicsLib.MAX_DIFFERENTIAL_LAYERS) {
            ...
            if (percent > previousPercent) {
                ...
                uint256 baseAmount = staticTotal;
                if (baseAmount > uplineTicket.amount) {
                    baseAmount = uplineTicket.amount;
                }
                uint256 rewardTotal = (baseAmount * diffPercent) / 100;
                ...
                // 存储MC和JBC分别的金额（需要扩展PendingReward结构或使用映射）
                // 暂时使用amount存储总额，在_releaseDifferentialRewards中按比例分配
                emit DifferentialRewardRecorded(stakeId, current, rewardTotal);
    ...
    function _distributeReward(address user, uint256 amount, uint8 rType) internal returns (uint256) {
        ...
        if (rType == REWARD_DIFFERENTIAL) {
            // 级差奖励应该通过 _distributeDifferentialRewardFromStatic 分配
            // 这里不应该被调用，因为级差奖励应该在赎回时从静态奖励中分配
            // 为了兼容性，使用50/50分配方式
    ...
    function _distributeDifferentialRewardFromStatic(address user, uint256 /* amount */, uint256 mcAmount, uint256 jbcAmount) internal returns (uint256) {
        UserInfo storage u = userInfo[user];
        ...
        if (jbcAmount > 0 && jbcToken.balanceOf(address(this)) >= jbcAmount) {
            ...
            uint256 jbcValueNeeded = (jbcAmount * jbcPrice) / 1 ether;
            if (jbcValueNeeded > 0 && swapReserveMC >= TokenomicsLib.MIN_LIQUIDITY && swapReserveJBC >= TokenomicsLib.MIN_LIQUIDITY) {
                uint256 numerator = jbcValueNeeded * swapReserveJBC;
                uint256 denominator = swapReserveMC + jbcValueNeeded;
                uint256 jbcOutput = numerator / denominator;
                
                uint256 priceImpact = (jbcValueNeeded * 10000) / swapReserveMC;
                if (priceImpact <= TokenomicsLib.MAX_PRICE_IMPACT && jbcOutput > 0) {
                    uint256 tax = (jbcOutput * swapBuyTax) / 100;
                    uint256 amountToUser = jbcOutput - tax;
                    
                    if (jbcToken.balanceOf(address(this)) >= jbcOutput && amountToUser > 0 && address(this).balance >= jbcValueNeeded) {
                        swapReserveMC += jbcValueNeeded;
                        swapReserveJBC -= jbcOutput;
                        jbcToken.burn(tax);
                        jbcToken.transfer(user, amountToUser);
                        jbcTransferred = amountToUser;
                    }
                }
            }
            
            // 如果交换失败，直接转账MC
            if (jbcTransferred == 0 && jbcValueNeeded > 0 && address(this).balance >= jbcValueNeeded) {
                _transferNativeMC(user, jbcValueNeeded);
                mcTransferred += jbcValueNeeded;
            }
        }
    ...
    function _releaseDifferentialRewards(uint256 stakeId) internal {
        ...
        for (uint256 i = 0; i < rewards.length; i++) {
            // 从静态奖励的MC和JBC中按比例分配级差奖励
            uint256 paid = _distributeDifferentialRewardFromStatic(
                rewards[i].upline, 
                rewards[i].amount,
                rewards[i].mcAmount,
                rewards[i].jbcAmount
